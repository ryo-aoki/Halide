...
public:
    ...
    inline Func BoxFilterY(bool auto_schedule, const Func& input, const Expr& radius,
        ...
        if (!auto_schedule) {
            ...
            output
                ...
                .vectorize(x_vi)
                .parallel(z);
            std::cout << "OUTPUT UPDATE SIZE: " << output.num_update_definitions() << "\n";
            for (int stage = 0; stage < 4; stage++) {
                output.update(stage)
                    .split(x, x_vo, x_vi, vector_size)
                    .vectorize(x_vi)
    ...
    Func ShallowLowerBound(bool auto_schedule,
        ...
        global_min(x) = Halide::undef<float>();
        global_min(0) =
            minimum(select(valid(r.x, r.y, r.z), input(r.x, r.y, r.z), kMaxVal));

        // Determine if any inputs are valid, which tells us whether or not to trust
        // global_min.
        Func any_valid("any_valid");
        any_valid(x) = Halide::undef<bool>();
        any_valid(0) = maximum(valid(r.x, r.y, r.z));
        ...
        min_val_fixed(x, y, z) =
            select(min_val(x, y, z) >= kMaxVal, global_min(0), min_val(x, y, z));
        ...
        lower_bound_clamped(x, y, z) =
            select(any_valid(0), lower_bound(x, y, z), kInfinity);
